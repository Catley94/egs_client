...
pub async fn download_asset(dm: &DownloadManifest, _base_url: &str, out_root: &Path) -> Result<(), anyhow::Error> {
    ...
    let mut up_to_date_files: usize = 0;
    ...
    if downloaded_files == 0 {
        if up_to_date_files > 0 {
            // All files were already present and validated; treat as success
            eprintln!(
                "Note: all files already present ({} up-to-date, {} with zero chunks)",
                up_to_date_files, skipped_files
            );
        } else {
